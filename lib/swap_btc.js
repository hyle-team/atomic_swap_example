 ...
 */
 ...
 const SwapBTCLowLevel = require('../lib/swap');
...
class SwapEntityBTC {
  ...
  {
    /*
    const btcFundingWallet = await this.btcWalletClient.wallet(this.walletID);
    const history = await btcFundingWallet.getHistory();
    const history_pending = await btcFundingWallet.getPending();
    const info = await btcFundingWallet.getInfo(); 
    const account =  btcFundingWallet.getAccount(this.walletAcct);
    //const pub_key = account.receiveKey();

    console.log('history: \n' + JSON.stringify(history));
    console.log('history_pending: \n' + JSON.stringify(history_pending));
    console.log('info: \n' + JSON.stringify(info));
    */
